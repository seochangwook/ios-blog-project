...
{
    BOOL is_delete = true; //처음은 실패라 가정//
    ...
    sqlite3_stmt *delete_stmt; //데이터베이스 수정작업을 위한 것.//
    char *delete_sql; //쿼리문//
    
    delete_sql = "DELETE FROM game_member WHERE member_id = ?";
    
    if(sqlite3_prepare_v2(database, delete_sql, -1, &delete_stmt, NULL) == SQLITE_OK)
    {
        //bind로 '?'에 값을 할당해준다. PrepareStatement방식 적용//
        sqlite3_bind_text(delete_stmt, 1, [inputid UTF8String], -1, SQLITE_TRANSIENT);
